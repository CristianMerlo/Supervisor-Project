# Paquete de Despliegue: Fase 3 e Inicialización de Sábana

**Instrucciones para el Antigravity de la máquina destino:**
Hola colega. Hemos llegado a la Fase 3. Nuestro objetivo aquí es conectar el `motor_supervisor.py` (que ya tienes configurado) con Google Sheets, y además cargar la nueva Base de Datos Maestra de Locales que el usuario acaba de proveer.

A continuación, sigue los pasos en orden. Requieres interacción humana para el Paso 1.

---

## PASO 1: Interacción Humana (Generar Service Account)
Pídele al usuario (Cristian) que realice los siguientes pasos manuales en su navegador, ya que nosotros no podemos acceder a su consola de Google Cloud por motivos de seguridad:

1. Ingresar a https://console.cloud.google.com/
2. Crear un Proyecto Nuevo (ej. `Supervisor-Hermes`).
3. Ir a "APIs y servicios" -> "Biblioteca" y habilitar **Google Sheets API** y **Google Drive API**.
4. Ir a "Credenciales" -> **+ CREAR CREDENCIALES** -> "Cuenta de servicio". Ponerle un nombre.
5. Hacer clic en el correo de la cuenta de servicio creada, ir a **CLAVES** -> **AGREGAR CLAVE** -> **Crear clave nueva (JSON)**.
6. **Guardar el archivo descargado como `credentials.json` en este mismo directorio.**
7. (Crucial) El usuario debe ir a su Google Sheet "La Sábana", hacer clic en Compartir, y darle permisos de **Editor** al correo de la cuenta de servicio.

*NOTA PARA ANTIGRAVITY: Espera a que el usuario confirme que `credentials.json` está en la carpeta antes de ejecutar el Paso 2 y 3.*

---

## PASO 2: Script de Actualización de Locales (`actualizar_locales.py`)
Una vez que tengas el `credentials.json` y el link del Google Sheet, crea y ejecuta este script UNA SOLA VEZ para volcar la base de datos maestra que el usuario nos proveyó en la pestaña `Locales_Maestro`.
...
SHEET_URL = "PEGAR_AQUI_LA_URL_DE_LA_SABANA" 
...
FMSNI|FMSNI|Melisa Castillo|Sabrina Orlando|SAN NICOLAS|sannicolas@mostazaweb.com.ar|Mitre 449|San Nicolás|BUENOS AIRES|Auto Mostaza|PUA NANI SRL
...
## PASO 3: Script de Inyección para el Motor (`fase3_sheets.py`)
Crea el script que el `motor_supervisor.py` usará para volcar los reportes en la pestaña `Historial_Mantenimiento` cada vez que lea un PDF. Guárdalo como `fase3_sheets.py`.
...
def inyectar_en_sabana(datos_extraidos, alertas_negocio, sheet_url):
    ...
    try:
        ...
        hoja_historial = sabana.add_worksheet(title="Historial_Mantenimiento", rows="1000", cols="15")
        # Escribir Headers
    ...
    if alertas_negocio.get('alertas_activas'):
        try:
            ...
            hoja_alertas = sabana.add_worksheet(title="Alertas_Activas", rows="1000", cols="10")
...
**Conclusión:** 
Antigravity destino, cuando el usuario confirme que creó el archivo `credentials.json` y colocó el link de la Sábana en tu configuración, instala `gspread` (`pip install gspread google-auth`), corre el script del Paso 2, y deja listo el script del Paso 3. ¡Terminado!
